                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    ...
                    <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
                        <div class="relative flex focus-within:z-10">
                            ...
                            <div class="relative mr-4 w-full flex-1" :class="{ hidden : !events }">
                                    ...
                                    <div class="overflow-y-auto">
                                        <table class="min-w-full border-separate border-spacing-0">
                                            ...
                                            <tr>
                                                ...
                                                >
                                                    Location
                                                    <span v-if="sort === 'location'">
                                                        {{ direction ? '⬆️' : '⬇️' }}
                                                    </span>
                                                </th>
                                                <th
                                                    @click="updateSort('starts_at')"
                                                    class="cursor-pointer top-0 z-10 border-b border-gray-300 bg-white/75 px-3 py-3.5 text-left whitespace-nowrap text-sm font-semibold text-gray-900 backdrop-blur backdrop-filter"
                                                >
                                                    Starts At
                                                    <span v-if="sort === 'starts_at'">
                                                        {{ direction ? '⬆️' : '⬇️' }}
                                                    </span>
                                                </th>
                                                <th
                                                    @click="updateSort('ends_at')"
                                                    class="cursor-pointer top-0 z-10 border-b border-gray-300 bg-white/75 px-3 py-3.5 text-leftwhitespace-nowrap text-sm font-semibold text-gray-900 backdrop-blur backdrop-filter"
                                                >
                                                    Ends At
                                                    <span v-if="sort === 'ends_at'">
                                                        {{ direction ? '⬆️' : '⬇️' }}
                                                    </span>
                                                </th>
                                                <th
                                                    @click="updateSort('ticket_limit')"
                                                    class="cursor-pointer top-0 z-10 border-b border-gray-300 bg-white/75 px-3 py-3.5 text-left whitespace-nowrap text-sm font-semibold text-gray-900 backdrop-blur backdrop-filter"
                                                >
                                                    Ticket Limit
                                                    <span v-if="sort === 'ticket_limit'">
                                                        {{ direction ? '⬆️' : '⬇️' }}
                                                    </span>
                                                </th>
                                                <th
                                            ...
                                            <tbody>
                                            <tr v-for="(event, eventIdx) in events" :key="event.email">
                                                <td :class="[eventIdx !== events.length - 1 ? 'border-b border-gray-200' : '', 'whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6 lg:pl-8']">{{ event.name }}</td>
                                                <td :class="[eventIdx !== events.length - 1 ? 'border-b border-gray-200' : '', 'hidden px-3 py-4 text-sm text-gray-500 lg:table-cell']"
                                                >{{ event.slug }}</td>
                                                <td :class="[eventIdx !== events.length - 1 ? 'border-b border-gray-200' : '', 'px-3 py-4 text-sm text-gray-500']"
                                                >{{ event.location }}</td>
                                                <td :class="[eventIdx !== events.length - 1 ? 'border-b border-gray-200' : '',
                                                ...
                                                <td :class="[eventIdx !== events.length - 1 ? 'border-b border-gray-200' : '',
                                                    ...
                                                    <span :class="[
                                                        ...
                                                        event.is_active ? 'bg-green-500' : 'bg-red-500'
                                                    ]"></span>
                                                </td>
                                            </tr>
                                            </tbody>
...
const refreshData = (overrides = {}) => {
    router.get(route('admin.dashboard.events'), {
        sort: props.sort,
        ...
        only: ['events', 'sort', 'direction', 'filters', 'search'],
        preserveState: true,
        preserveScroll: true,
    })
};
...
function updateSort(field) {
    ...
    const sortField = isSameField && !props.direction ? 'starts_at' : field; // Resets to name on 3rd click
